...
<!-- 	            <button class="btn btn-primary" id="emp_add_modal_btn">新增</button>
 ...
 -->	        </div>
				...
				<!-- <button class="btn btn-primary" id="emp_add_modal_btn">查看生成词云</button> -->
				   <div class="row">
					    <div class="input-group">
					      <input type="text" name="userName" class="form-control" placeholder="请输入发送者昵称...">
				...
				<table class="table table-hover" id="rough_table">
					<thead>
				  	...
				  	<colgroup>
					    ...
					    <col width='10%'></col> <!--发送者ID -->
					    <col width='15%'></col><!-- 发送者昵称 -->
					    <col width='10%'></col><!-- 接收者ID -->
					    <col width='15%'></col><!-- 接收者昵称 -->
					    <col width='15%'></col><!-- 通信时间 -->
					    <col width='20%'></col><!-- 查看详情 -->
	            ...
	            <div class="modal-body">
	                 <form class="form-horizontal" action="upload/client_upload_userImage_to_oss" method="post"  enctype="multipart/form-data"> 
		                <div class="form-group">
	                        ...
	                        <div class="col-sm-10">
	                            <input type="text" name="userId" class="form-control" id="userId" placeholder="用户昵称">
	                            <span class="help-block"></span>
	                        </div>
	                    </div>	
	
						<div class="form-group">
	                        <label for="userName_add_input" class="col-sm-2 control-label">选择头像</label>
	                        <div class="col-sm-10">
	...
	function to_page(pn) {
		...
		$.ajax({
			url : "/Picture_SpringMvc/chat/findAll_Messge",
			data : "pn=" + pn,
			type : "GET",
		...
		$
				.each(
						...
						function(index, item) {
							...
							//alert(item.userName);
							var senderIDTD = $("<td></td>").append(item.senderID);
							var senderNameTD = $("<td></td>").append(item.senderName);
							var receiverIDTD = $("<td></td>").append(item.receiverID);
							var receiverNameTD = $("<td></td>").append(item.receiverName);
							var chatTimeTD = $("<td></td>").append(item.chatTime);
							var chatContentTD = $("<td></td>").append(item.chatContent);
							/* 数据来源在数据详情显示 */
							...
							urlBtn.attr("url-id", item.picURL);
							...
							$("<tr></tr>").append(checkboxTd).append(
									senderIDTD).append(senderNameTD).append(receiverIDTD).append(receiverNameTD)
									.append(chatTimeTD).append(chatContentTD)
	...
	function get_job_by_key() {
		...
		$.ajax({
			url : "/Picture_SpringMvc/chat/get_chat_byUserName",
