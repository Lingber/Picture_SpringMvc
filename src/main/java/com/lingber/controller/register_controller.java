...
public class register_controller {
    ...
    public Msg_bean register_client(String username,String phonenumbRegister,String code,String RegisterpassWord){
		if (code.equals(this.SMS_Code)) {//判断验证码是否正确
			//新建用户
			User_Bean user_Bean = new User_Bean();//新建userBean
			...
			user_Bean.setUserID(userID_Radom_tool.UUID_UserID());
	...
	//服务器专用
	@ResponseBody
    @RequestMapping("/register_request_server")  
    public Msg_bean register_server(String userName,String passWord,String phone_Numb){
		Query query_phone =new Query(Criteria.where("phone_Numb").is(phone_Numb));
		User_Bean find_phone = mongoTemplate.findOne(query_phone, User_Bean.class, "mvc");
		if (find_phone==null) {//判断验证码是否正确
			//新建用户
			User_Bean user_Bean = new User_Bean();
			user_Bean.setUserName(userName);
			user_Bean.setPhone_Numb(phone_Numb);
			//生成user_id
			UserID_Radom_tool userID_Radom_tool =new UserID_Radom_tool();
			user_Bean.setUserID(userID_Radom_tool.UUID_UserID());
			...
			mongoTemplate.insert(user_Bean, "mvc");
			System.out.println("插入成功！");
			Msg_bean msg_bean =new Msg_bean("04", "注册成功!",null);
			return msg_bean;
		}else{
